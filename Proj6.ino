...
pd_step(){
    imu::Vector<3> IMU = bno.getVector(Adafruit_BNO055::VECTOR_EULER);
float fx = 0.0;
float fy = -0.0;
float kd = 0.05 //derivative gain
float torque = kd * IMU.z;
set_hovercraft_forces(fx,fy,torque);
//create once per second output loop for the rotation rate when the function is called
while(millis() - last_fsm_time > 1000){
...
// STATE_LIFT_UP: set central fan to a 25-40% duty cycle (255 = 100% duty cycle)
        case STATE_LIFT_UP:
            fan.write(90);
            pd_step(); //should call the PD step function and allow us to resist rotation duing the run time of the code

// after 600 cycles (30 seconds), transition to the next state (STATE_POSITIVE_TORQUE) and reset the counter
            if (++counter >= 600) {
...
// STATE_FANS_OFF_1: turn off the central fan and wait for 15 seconds
        case STATE_FANS_OFF_1:
            ...
            fan.write(0);
            // reset to start state and then begin to wait for switch to be pressed again.
