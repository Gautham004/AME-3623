...
void set_motor(int pwmPin, int inAPin, int inBPin, float val) {
    ...
    if      (val > 0) { digitalWrite(inAPin, HIGH); digitalWrite(inBPin, LOW ); }
    else if (val < 0) { digitalWrite(inAPin, LOW ); digitalWrite(inBPin, HIGH); }
    else              { digitalWrite(inAPin, LOW ); digitalWrite(inBPin, LOW ); }
