...
//------------------------------------------------------------------------------
bno.getCalibration(int system, int gyro, int accel, int mag);
...
float db_clip(float error, float deadband, float saturation) {
  ...
  void sensor_display() {
    ...
    if(distance_right <= 10){
      int base_strength_right = 10;
      base_strength_right = base_strength_right * (1/distance_right);
    }
    if(distance_left <= 10){
      int base_strength_left = 10;
      base_strength_left = base_strength_left * (1/distance_left);
    }
    if(distance_rear <= 10){
      int base_strength_rear = 10;
      base_strength_rear = base_strength_rear * (1/base_strength_rear);
    }
    //translate the strengths into hovercraft forces
    int horizontal_forces = base_strength_right - base_strength_left;
    int vertical_forces = base_strength_rear; 
    set_hovercraft_forces(horizontal_forces, vertical_forces, 0);
  }
  ...
  void pd_step() {
    ...
    float current_heading = event.orientation.z;  // Assuming z-axis is yaw
    float yawRate = g.z();                        // deg/s
    const float Kd = 0.25f;                       // derivative gain
    const float Kp = 0.25f;                       // needs to be tuned*****
    const float deadband = 0.5f;                  // deg/s threshold
    const float saturation = 45f;                 // deg saturation
