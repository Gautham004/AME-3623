...
#define FAN1_PWM 9 //3 blue is pwm 3
#define FAN1_INA 28 //32
#define FAN1_INB 27 //31

// defining the pwm, inA, and inB pins for Fan 2
#define FAN2_PWM 3//6 
#define FAN2_INA 32 //30
#define FAN2_INB 31 //29

// defining the pwm, inA, and inB pins for Fan 3
#define FAN3_PWM 6 //9
#define FAN3_INA 30 //28
#define FAN3_INB 29 //27
...
void set_hovercraft_forces(float fx, float fy, float torque) {
  ...
  */
  ...
  motor_values[0] =  (fx/(2*(cos(30)+1))) + (fy/(2*(sin(30)))) + (torque/(2*R*(cos(30)+1)));   //(fx/(2*cos(30))) - (fy/(2+2*sin(30))) - (torque/(2*R*(1+sin(30))));  **for right motor
  motor_values[1] =  (fx/(2*(cos(30)+1))) + (fy/(2*(sin(30)+1))) -  (torque/(2*R*(cos(30)+1)));      //(fx/(2*cos(30))) + (fy/(2+2*sin(30))) + (torque/(2*R*(1+sin(30)))) * 1.2;
  motor_values[2] =  (fx/(cos(30)+1)) - ((torque*cos(30))/(R*(cos(30)+1)));  //(-fy/(1+sin(30))) + (torque * sin(30)/(R*(1 + sin(30)))) * 1.2;
...
// declaring the negative gain array
    const float negative_gain[3] = {1.0, 1.3 , 1.2};
...
// after 300 cycles (15 seconds), transition to the next state (STATE_POSITIVE_TORQUE) and reset the counter
            if (++counter >= 300) {
                currentState = STATE_POSITIVE_TORQUE; //BANDAID FIX - is normally positive torque
                counter = 0;
            }
            break;
...
// after 300 cycles (15 seconds), transition to the next state (STATE_NEGATIVE_TORQUE) and reset the counter
            if (++counter >= 300) {
                currentState = STATE_NEGATIVE_TORQUE; //BANDAID FIX - is normally negative torque
                counter = 0;
            }
            break;
